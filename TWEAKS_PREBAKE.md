...
**Status:** superseded by the general `.psxmod` runtime. Retained as historical
analysis and as documentation for the parity tools below. See
`docs/MMX6_TWEAKS_MODS.md`.
**Goal achieved by a different mechanism:** changed guest code now executes from
live RAM through PSXRecomp's guarded fallback, so a profile is a boot-time
expected-byte overlay rather than a generated-C variant.
...
3. **Launcher (historical design; now shared Dear ImGui)**: tweaks menu → flags/
